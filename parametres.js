...
async function init(){
  ...
  if (profileError || !profile){
    ...
    paramsWrap.innerHTML = `<div class="empty-state">Impossible de charger ton profil.<br><small style="color:var(--txt-att);">${profileError ? escapeHtml(profileError.message) : 'Profil introuvable.'}</small></div>`;
    return;
  }
  ...
  paramsWrap.innerHTML = `
    ...
    <form id="schoolForm">
      <div class="form-group">
        ...
        ${schoolsList.length === 0 ? `<div class="form-hint">Il n'existe pas encore de liste pré-remplie pour l'urbanisme — choisis "Autre" et écris le nom de ton établissement.</div>` : ''}
      </div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (pendingAvatarFile){
        const path = `${currentUser.id}/${Date.now()}-${sanitizeFileName(pendingAvatarFile.name)}`;
